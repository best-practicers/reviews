...
/// @title Trade Squads
/// @author 
/// @notice contract defining the  underlying NFT asset  of NFTTrader application and supporting functions
/// @dev this asset is traded using meta-transaction gas fees.
contract TradeSquads is ERC721, Ownable, Pausable {
    ...
    /// @notice data structure storing the available tokens with the user .
    /// @dev used further as mapping(reqInfo => series).
    struct Series {
...
/// @notice mints an unique nft for the user who had traded for first time  or assigns the atteibutes of the token in the given series after corresponding time 
/// @dev use of bock.timestamp which might create possiblity of frontrunning , needs to be upgraded . 
/// @param  _series stores the  id for the specific token series  for checking the given user.
/// @param  _unique  whether the user is first time claimer of the nft token.
/// @return  null
    ...
    /// @notice determines the property for the particular token and its fixed number of trait and returns them as tuple. 
    /// @dev currently not being used in any contract , thus doesnt affect the workflow
    /// @param _tokenId 721 compliant identifier for the project .
    /// @return tuple of tokendetail and its trait values .
